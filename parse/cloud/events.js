				...
				runTest = function(e, combo, opt) {
					...
					if (opt['disable_in_input']) {
						...
						if (element && element.tagName && (element.tagName.toLowerCase() === 'input' ||
							element.tagName.toLowerCase() === 'textarea') && element !== opt.target) {
							return;
						}
					}
					...
					mods = modifiers();
					if (e.ctrlKey) {
						mods.ctrl.pressed = true;
					}
					if (e.shiftKey) {
						mods.shift.pressed = true;
					}
					if (e.altKey) {
						mods.alt.pressed = true;
					}
					if (e.metaKey) {
						mods.meta.pressed = true;
					}
		...
		self2.run = function(evt, controller) {
			var controller = controller || new EventController(evt, target),
				...
				eventTest = (event in specialEvents && specialEvents[event].eventTest) ?
					specialEvents[event].eventTest : function() {
						return true;
				},
	...
	self.buildEventObject = (function() {
				...
				for (var i in eventTypes) {
					if (eventTypes.hasOwnProperty(i)) {
						if (contains(type, eventTypes[i])) {
							...
							if (i === 'KeyEvents' && !window.KeyEvent) {
								i = 'UIEvents';
							}
							if (document.implementation.hasFeature(i, '2.0') || window[i.substring(0, i.length - 1)]) {
								i = 'Events';
							}
		...
		if (document.createEvent) {
			return function(type, info, extra) {
				...
				if (typeof origInfo !== 'object' || info.useDefaults) {
					module = 'Events';
				}
				switch (module) {
					case 'Events':
